    ...
    <S as ToOwned>::Owned: fmt::Debug,
{
    ...
    /// use nu_ansi_term::AnsiGenericString;
    /// use std::path::Path;
    /// let icon_string = AnsiGenericString::icon(Path::new("foo/bar.icn").to_string_lossy());
    /// println!("{}", icon_string);
    /// ```
    /// Should produce an empty line but set the terminal icon.
    /// Notice that we use std::path to be portable.
    pub fn icon<I>(s: I) -> Self
    where
        I: Into<Cow<'a, S>>,
    {
        Self {
            style: Style::default(),
            string: s.into(),
            oscontrol: Some(OSControl::<'a, S>::Icon),
        }
    }
        ...
        I: Into<Cow<'a, S>>,
    {
        Self {
            style: Style::default(),
            string: s.into(),
            oscontrol: Some(OSControl::<'a, S>::Cwd),
        }
    }
    ...
    &'a S: AsRef<[u8]>,
{
    ...
    fn write_inner<W: AnyWrite<Wstr = S> + ?Sized>(&self, w: &mut W) -> Result<(), W::Error> {
        // \x01 .. \x02 are used to wrap zero-width portions of output
        match &self.oscontrol {
...
mod tests {
    ...
    #[test]
    fn icon() {
        let icon = AnsiGenericString::icon("/path/to/test.icn");
        assert_eq!(icon.to_string(), "\x01\x1B]I;/path/to/test.icn\x1B\\\x02");
        idempotent(icon)
    }

    #[test]
    fn cwd() {
        let cwd = AnsiGenericString::cwd("/path/to/test/");
        assert_eq!(cwd.to_string(), "\x01\x1B]7;/path/to/test/\x1B\\\x02");
        idempotent(cwd)
    }
